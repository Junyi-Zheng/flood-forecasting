...
These cover the in-process pieces of the pipeline: accumulation differencing,
the WeatherBench 2 daily aggregation, the canonical batch schema, the Zarr
append and in-place write primitives, the missing-data fallback, and the
command line surface. Full builds live in
``test_gridded_archive_integration.py``.
"""
...
class TestDeaccumulate:
  ...
  def test_clip_negative_floors_precipitation_noise(self) -> None:
    """A cumulative total can dip slightly on rounding; rain cannot be < 0."""
    accumulated = np.array([5.0, 4.9, 7.0], dtype=np.float32)

    np.testing.assert_allclose(
        deaccumulate(accumulated, clip_negative=True), [5.0, 0.0, 2.1], atol=1e-6
    )

  def test_radiation_keeps_negative_increments(self) -> None:
    """Net thermal radiation is genuinely negative and must not be clipped."""
...
class TestWeatherBench2Aggregation:
  ...
  def source(self) -> WeatherBench2Source:
    """A WB2 source backed by a synthetic in-memory dataset.

    ``__new__`` bypasses ``__init__`` (which would open the real remote Zarr)
    so the aggregation logic can be tested without any network access.
    """
    ...
    # Temperature equals the lead hour, so each lead day's 24h mean is the
    # mean of hours {d*24-18, d*24-12, d*24-6, d*24} = d*24 - 9.
  ...
  def test_precipitation_is_taken_from_the_24h_accumulation(
    ...
    """WB2 already provides 24h totals, so no differencing should occur."""
    result = source.extract_date(pd.Timestamp("2020-03-01"))
  ...
  def test_radiation_arrays_are_independent_objects(
    ...
    """Both radiation fields are NaN-filled; they must not alias each other."""
    result = source.extract_date(pd.Timestamp("2020-03-01"))
...
class TestBuildBatchDataset:
  ...
  def test_attributes_are_copied_not_shared(self) -> None:
    """Mutating one batch's attrs must not corrupt the module-level template."""
    dataset = make_batch_dataset(["2020-01-01"])
...
class TestWriteBatchToZarr:
  ...
  def test_append_extends_the_time_axis(self, tmp_path: Path) -> None:
    ...
    write_batch_to_zarr(make_batch_dataset(["2020-01-02", "2020-01-03"]), target)
...
class TestWriteBatchInPlace:
  ...
  def test_untouched_dates_are_preserved(self, populated_store: str) -> None:
    ...
    with xr.open_zarr(populated_store, consolidated=False) as store:
      values = store["temperature_2m"].values
    # Dates 1, 3, 4, 5 keep their original per-index values (0, 2, 3, 4).
...
class TestSourceDispatch:
  ...
  Tests routing across WeatherBench 2 (<= 2023-01-10), the NaN-initialized gap
  window (2023-01-11 to 2023-07-12), and ECMWF Open Data (>= 2023-07-13).
  """
  ...
  def test_gap_window_emits_nan_slice(
    ...
    """Dates in the 2023-01-11..2023-07-12 window emit a contiguous NaN slice."""
    calls = self._install(monkeypatch)
...
class TestMissingDataFallback:
  ...
  def test_writes_a_full_nan_slice(
  ...
  def test_nan_slices_are_not_aliased(
    ...
    """Each variable needs its own buffer, or writes would bleed across them."""
...
class TestCommandLine:
  ...
  def test_rejects_unknown_flags(self) -> None:
    with pytest.raises(SystemExit):
      build_arg_parser().parse_args(["--not_a_real_flag"])
