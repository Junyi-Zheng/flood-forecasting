...
These cover the pure, in-process transformations: the NOAA PSL -> Caravan
MultiMet grid rewrite, date filtering, the Zarr write/append primitive, and the
command line surface. End-to-end builds live in
...
class TestGridStandardization:
  ...
  def test_latitude_is_flipped_to_ascending(self, sample_year: Path) -> None:
    ...
    # Strictly increasing, which xarray needs for label-based selection.
  ...
  def test_values_follow_their_coordinates(self, sample_year: Path) -> None:
    ...
    # Mark a single, unambiguous cell in PSL index space and check that it
    # reappears at the matching latitude/longitude after standardization.
    ...
    assert float(dataset[CPC_VARIABLE].isel(time=0).sum()) == pytest.approx(42.0)

  def test_missing_values_become_nan(self, sample_year: Path) -> None:
    dataset = process_cpc_netcdf_to_dataset(str(sample_year))
    values = dataset[CPC_VARIABLE].values

    # PSL (0, 0, 0) is lat +89.75, lon 0.25 -> lat index 359, lon index 360.
  ...
  def test_negative_sentinels_become_nan(
    ...
    """NOAA also uses plain negative numbers to flag unusable cells."""
...
class TestWriteBatchToZarr:
  ...
  def test_append_preserves_previously_written_values(
    ...
    write_batch_to_zarr(full.isel(time=slice(0, 1)), target, is_initial_write=True)
  ...
  def test_chunking_is_capped_at_thirty_days(
    ...
    """Short first batches must not lock the store into a tiny chunk size."""
    target = str(tmp_path / "cpc.zarr")
    first = process_cpc_netcdf_to_dataset(
        str(sample_year),
        target_end_date=pd.Timestamp("2020-01-02"),
...
class TestEnsurePslNetcdf:
  """Download caching. The network path itself is never exercised."""
